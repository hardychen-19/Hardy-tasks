...
- Deadline alert: a fixed rounded field begins at the physical screen boundary and occupies roughly 40 points inward. Twelve wide, translucent, overlapping `CAShapeLayer` bands light from outside to inside over three pulses without translating or morphing the screen outline; this keeps all four corners continuously rounded. Blue signals approaching, orange urgent, and red overdue. After 2.48 seconds, a compact 480-point task island shows the closest upcoming task for three seconds, then one overdue task plus an “N more” count for three seconds. Core Animation performs the wave on the GPU and all layers are removed when hidden.
