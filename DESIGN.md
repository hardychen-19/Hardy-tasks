...
- Deadline alert: one 1.15-point halo hugs the exact notch and completes a restrained 0.9-second breath without gradients. After 0.38 seconds, a compact 480-point task island grows from the notch, names the leading task, shows its deadline state, collapses concurrent cues into an “N more” count, and withdraws after three seconds. The 60 Hz animation timeline runs only while the halo is visible and pauses immediately afterward.
